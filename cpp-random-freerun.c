...
int main(int argc, char * argv[])
{
    ...
    snprintf(path_shard[0], PATH_BUFF_SIZE, "results/random-d0.%s.%s.results", core_type, fs_type);
    snprintf(path_shard[1], PATH_BUFF_SIZE, "results/random-d1.%s.%s.results", core_type, fs_type);
    snprintf(path_shard[2], PATH_BUFF_SIZE, "results/random-d2.%s.%s.results", core_type, fs_type);
    snprintf(path_shard[3], PATH_BUFF_SIZE, "results/random-d3.%s.%s.results", core_type, fs_type);
    ...
    iosize_actual[0] = MIN(fsize[0] / 2, IOSIZE);
    iosize_actual[1] = MIN(fsize[1] / 2, IOSIZE);
    iosize_actual[2] = MIN(fsize[2] / 2, IOSIZE);
    iosize_actual[3] = MIN(fsize[3] / 2, IOSIZE);

    u_int64_t seeklimit[4] = { 0 };

    seeklimit[0] = fsize[0] - iosize_actual[0];
    seeklimit[1] = fsize[1] - iosize_actual[1];
    seeklimit[2] = fsize[2] - iosize_actual[2];
    seeklimit[3] = fsize[3] - iosize_actual[3];
    ...
    for(; keepRunning && round < 4; ++round)
    {
        ...
        while(keepRunning && trials--)
        {
            ...
            while(writelen > 0)
            {
                u_int64_t offset = rand() % seeklimit[round];
                u_int64_t bytesWritten = pwrite(trialoutfd, randomnessCopy + offset, iosize_actual[round], fsize[round] - writelen);
            ...
            {
                u_int64_t offset = rand() % seeklimit[round];
                u_int64_t bytesRead = pread(trialoutfd, readback, iosize_actual[round], offset);
